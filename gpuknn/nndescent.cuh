...
struct NNDItem {
  ...
  NNDItem(int id, bool visited, float distance)
      : id(id), visited(visited), distance(distance) {}
  bool operator==(const NNDItem &other) const {
    return (this->id == other.id) &&
           (fabs(this->distance - other.distance) < 1e-9);
  }
  bool operator<(const NNDItem &other) const {
    if (fabs(this->distance - other.distance) < 1e-9)
      return this->id < other.id;
